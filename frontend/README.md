...
| Đại lý | `daily@rentflow.vn` | `123456` | `/admin/bat-dong-san` |
| Môi giới | `moigioi@rentflow.vn` | `123456` | `/admin/lich-xem-nha` |
| Khách hàng | `khachhang@rentflow.vn` | `123456` | `/tenant` |
...
frontend/
  ...
  src/
    ...
    services/
      api.js
      authService.js
      batDongSanService.js
      chuNhaService.js
      khachHangService.js
      hopDongKyGuiService.js
      hopDongThueService.js
      lichHenKhaoSatService.js
      lichHenXemNhaService.js
      contractService.js
      taiChinhService.js
      nhanVienService.js
      quanTriService.js
...
Các route này dùng `TenantLayout` và yêu cầu `userInfo.role === 'KHACH_HANG'`.
...
- `ADMIN` -> `/admin`
- `CHU_NHA` -> `/dashboard`
- `KHACH_HANG` -> `/tenant` trực tiếp hoặc thông qua redirect từ `/`
...
| `KHACH_HANG` | Redirect về `/tenant` khi vào `/` | Được xem | Được xem chi tiết | Không hiện link, không được vào form | `/tenant` |
...
FE dùng axios client chung trong `src/services/api.js` (base URL từ env, tự gắn token + refresh token). Mỗi nghiệp vụ có 1 service file riêng:

| Service file | Endpoint chính | Gọi bởi trang |
| --- | --- | --- |
| `api.js` | Axios instance, auth interceptor | Tất cả service khác |
| `authService.js` | `/api/auth/*` | `AuthContext` (qua hook `useAuth`) |
| `batDongSanService.js` | `/api/bat-dong-san/*` | AdminBatDongSanPage, ChiTietQuanLyBatDongSanPage, DangKyKyGuiPage, QuanLyBatDongSanPage, AdminLichKhaoSatPage |
| `chuNhaService.js` | `/api/chu-nha/*` | AdminChuNhaPage, QuanLyBatDongSanPage |
| `khachHangService.js` | `/api/khach-hang/*` | CustomersPage, ChiTietKhachHangPage, PhanCongMoiGioiPage |
| `hopDongKyGuiService.js` | `/api/hop-dong-ky-gui/*` | HopDongKyGuiPage, ChiTietHopDongKyGuiPage, ChiTietQuanLyBatDongSanPage, AdminHopDongKyGuiPage, LegalPage |
| `hopDongThueService.js` | `/api/hop-dong-thue/*` | LegalPage, TenantDashboardPage |
| `lichHenKhaoSatService.js` | `/api/lich-hen-khao-sat/*` | AdminLichKhaoSatPage, LichKhaoSatPage, ChiTietQuanLyBatDongSanPage |
| `lichHenXemNhaService.js` | `/api/lich-hen-xem-nha/*` | LichXemNhaPage, PhanCongMoiGioiPage |
| `contractService.js` | `/api/hop-dong-ky-gui/*`, `/api/hop-dong-thue/*` | AdminHopDongKyGuiPage, AdminHopDongThuePage, LegalPage |
| `taiChinhService.js` | `/api/tai-chinh/*` | HoaHongPage, AdminDashboardPage |
| `nhanVienService.js` | `/api/nhan-vien-moi-gioi/*` | PhanCongMoiGioiPage |
| `quanTriService.js` | `/api/quan-tri/*` | AccountManagementPage |
| `baoCaoService.js` | `/api/bao-cao/*` | BaoCaoThongKePage, AdminDashboardPage |
| `viewingService.js` | `/api/lich-hen-xem-nha/*`, `/api/bat-dong-san-cong-khai/*` | BatDongSanPage, ChiTietBatDongSanPage, HomePage, TenantDashboardPage, TenantFindPropertyPage, ScheduleViewingPage, MyViewingSchedulePage, SavedPropertiesPage |

### Trạng thái tích hợp theo luồng

#### Đã nối API thật (28 trang)

| Luồng | Trang | Service |
| --- | --- | --- |
| **Public** | HomePage | viewingService |
| | BatDongSanPage | viewingService |
| | ChiTietBatDongSanPage | viewingService |
| | RegisterPage | api (trực tiếp) |
| **Khách hàng** | TenantDashboardPage | viewingService, hopDongThueService |
| | TenantFindPropertyPage | viewingService |
| | SavedPropertiesPage | viewingService |
| | MyViewingSchedulePage | viewingService |
| | ScheduleViewingPage | viewingService |
| **Chủ nhà** | QuanLyBatDongSanPage | batDongSanService, chuNhaService |
| | ChiTietQuanLyBatDongSanPage | batDongSanService, hopDongKyGuiService, lichHenKhaoSatService |
| | DangKyKyGuiPage | batDongSanService |
| | HopDongKyGuiPage | hopDongKyGuiService |
| | ChiTietHopDongKyGuiPage | hopDongKyGuiService |
| | LichKhaoSatPage | lichHenKhaoSatService |
| **Admin** | AdminDashboardPage | baoCaoService, taiChinhService |
| | AdminChuNhaPage | chuNhaService |
| | AdminBatDongSanPage | batDongSanService |
| | AdminHopDongKyGuiPage | contractService, hopDongKyGuiService |
| | AdminHopDongThuePage | contractService |
| | AdminLichKhaoSatPage | lichHenKhaoSatService, batDongSanService |
| | LichXemNhaPage | lichHenXemNhaService |
| | PhanCongMoiGioiPage | khachHangService, nhanVienService, batDongSanService, lichHenXemNhaService |
| | HoaHongPage | taiChinhService |
| | TienDamBaoPage | (dùng chung view với admin) |
| | BaoCaoThongKePage | baoCaoService |
| | LegalPage | contractService, hopDongKyGuiService, hopDongThueService |
| | AccountManagementPage | quanTriService |
| | CustomersPage | khachHangService |
| | ChiTietKhachHangPage | khachHangService |

#### Chưa nối API — vẫn dùng mock data (8 trang)

| Luồng | Trang | Mock data | Ghi chú |
| --- | --- | --- | --- |
| **Chủ nhà** | DashboardPage | `MOCK_PROPERTIES`, `MOCK_SURVEYS`, `MOCK_NOTIFICATIONS` | Chuyển sang `batDongSanService.theoChuNha()`, `lichHenKhaoSatService.theoChuNhaHienTai()` |
| | ThongBaoPage | `NOTIFICATIONS` | Cần API thông báo |
| | TienDamBaoPage | `STATS_DATA`, `TRANSACTIONS` | Cần API tài chính cho chủ nhà |
| **Khách hàng** | TenantProfilePage | `MOCK_USER`, `MOCK_PREFERENCES`, `MOCK_ACTIVITY`, `MOCK_DEVICES` | Cần API profile CRUD |
| | TenantContractsPage | `MOCK_CONTRACTS` | Cần `hopDongThueService` cho tenant |
| | NotificationsPage | `MOCK_NOTIFICATIONS` | Cần API thông báo |
| **Admin** | CaiDatPage | `SYSTEM_INFO` | Trang cấu hình, chưa cần gấp |
| | HoSoCaNhanPage | Đọc `localStorage` | Cần API profile CRUD + đổi mật khẩu |

#### Chưa hoàn thành nối API (1 trang)

| Luồng | Trang | Ghi chú |
| --- | --- | --- |
| **Admin** | TaoHopDongThuePage | Có `useEffect` TODO gọi `lichHenXemNhaService.chiTiet()`, `handleSubmit` TODO gọi `hopDongThueService.tao()` — cả 2 đang comment, chỉ `console.log` |
...
- Gọi API qua service layer trong `src/services/`, mỗi nghiệp vụ 1 file. Không gọi `fetch` hay `axios` trực tiếp trong page component.
...
- 28/37 trang đã nối API thật qua service layer (`src/services/`), 8 trang vẫn dùng mock data, 1 trang đang dở nối API.
- Các trang chưa nối API tập trung ở 3 luồng: Dashboard chủ nhà (3 trang), Tenant (3 trang), Admin settings/profile (2 trang).
- `TaoHopDongThuePage` đã import service nhưng call API đang comment — cần bật lại.
- Base URL API đã lấy từ env (`VITE_API_BASE_URL`), fallback `http://localhost:8080/api`.
