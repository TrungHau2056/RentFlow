  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (isInternalAdminRole(data.data.user?.role)) navigate('/admin')
        else if (data.data.user?.role === 'CHU_NHA') navigate('/dashboard')
        else if (data.data.user?.role === 'KHACH_THUE') navigate('/tenant')
        else navigate('/')
      } else {
