...
function ResultModal({ schedule, onClose, onSave, loading }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await onSave(schedule.id, ketQua, nhanXet, ghiChuMG)
      onClose()
    } catch {
      setSaving(false)
    }
...
export default function LichXemNhaPage() {
  ...
  const handleSaveResult = async (id, ketQuaVal, nhanXet, ghiChuMG) => {
    ...
    try {
      ...
      setResultModal(null)
      if (ketQuaVal === 'dong_y_thue') {
        navigate(`/admin/hop-dong-thue/tao-moi?lichXemId=${id}`)
      } else {
        fetchSchedules()
      }
    } catch (err) {
