      ...
      <td className="py-3 px-4">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center shrink-0">
            <span className="text-[10px] font-bold text-white">{tenMG.split(' ').map(n => n[0]).join('').slice(-2)}</span>
          </div>
      ...
      <div className="p-5 space-y-5 max-h-[calc(100vh-220px)] overflow-y-auto">
          ...
          <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center shrink-0">
                <span className="text-white text-sm font-bold">{tenMG.split(' ').map(n => n[0]).join('').slice(-2)}</span>
              </div>
        ...
        <div className="border-t border-slate-200 pt-4 space-y-2">
          {comm.trangThai === 'cho_thanh_toan' && (
            <button
              onClick={(e) => { e.stopPropagation(); onMarkPaid(comm.id) }}
            ...
            >
              {actionLoading === comm.id && <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />}
              Duyệt thanh toán
            </button>
          )}
          <Link
...
function mapApiCommissionToLocal(c) {
  ...
  return {
    ...
    khachThue: '',
    chuNha: '',
    giaTriHD: Number(c.soTienHopDong),
    thoiHan: 12,
    phanTram: 5,
    soTien: Number(c.soTienHoaHong),
    trangThai: trangThaiMap[c.trangThaiThanhToan] || 'cho_thanh_toan',
    ngayTao: c.ngayTinh || '',
    ngayThanhToan: null,
    ghiChu: c.trangThaiThanhToan === 'KHAU_TRU' ? 'Đã khấu trừ tiền đảm bảo' : '',
...
export default function HoaHongPage() {
  ...
  const handleThanhToan = useCallback(async (id) => {
    setActionLoading(id)
    try {
  ...
  const handleTinhHoaHong = useCallback(async (hopDongThueId) => {
    setActionLoading('tinh')
    try {
      await taiChinhService.tinhHoaHong(hopDongThueId)
      fetchData()
      alert('Tính hoa hồng thành công')
    } catch (err) {
  ...
  const selectedComm = useMemo(() => commissions.find(c => c.id === selectedId), [selectedId, commissions])
  ...
  const kpi = useMemo(() => {
    ...
    const successHD = commissions.filter(c => c.trangThai === 'da_thanh_toan').length
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => {
              const hdId = prompt('Nhập ID hợp đồng thuê để tính hoa hồng:')
              if (hdId) handleTinhHoaHong(parseInt(hdId, 10))
            }}
      ...
      <div className="flex gap-6">
        ...
        {selectedId && (
          <div className="hidden xl:block w-105 shrink-0">
            <CommissionDetail comm={selectedComm} onClose={() => setSelectedId(null)} onMarkPaid={handleThanhToan} actionLoading={actionLoading} />
          </div>
        )}
      </div>
    </div>
